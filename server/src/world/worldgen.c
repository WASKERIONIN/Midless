...
bool Worldgen_Freeze(void) {
    ...
    if (FileExists("world/worldgen.meta")) {
        ...
        /* Legacy world (no worldgen.meta): its chunks were generated by a
         * different generator. Wipe them so the new worldgen applies. */
        FilePathList files = LoadDirectoryFiles("world");
        for (unsigned int i = 0; i < files.count; i++) {
            const char *name = GetFileName(files.paths[i]);
            if (!name || !strcmp(name, "seed.dat")) continue;  /* keep the world seed */
            const char *ext = GetFileExtension(name);
            if (ext && (!strcmp(ext, ".dat") || !strcmp(ext, ".bin"))) {
        ...
        UnloadDirectoryFiles(files);
        if (legacyWorld)
            TraceLog(LOG_WARNING, "Legacy world chunks removed; a fresh cosmic world will generate.");
    }
