...
static const I18nEntry table[] = {
    ...
    { "Server '%s' listening on %s:%d - send this address to friends. F6 - host panel.", "Сервер '%s' слушает %s:%d - дайте этот адрес друзьям. F6 - панель хоста.", "服务器 '%s' 监听 %s:%d - 把地址发给朋友。F6 - 主机面板。", "サーバー '%s' は %s:%d で待受中 - 友達にこのアドレスを。F6 - ホストパネル。", "서버 '%s' 수신 중 %s:%d - 친구에게 주소를 알리세요. F6 - 호스트 패널." },
    { "HOST %s:%d  players %d/%d", "ХОСТ %s:%d  игроков %d/%d", "主机 %s:%d  玩家 %d/%d", "ホスト %s:%d プレイヤー %d/%d", "호스트 %s:%d 플레이어 %d/%d" },
    { "HOST PANEL", "ПАНЕЛЬ ХОСТА", "主机面板", "ホストパネル", "호스트 패널" },
    { "server: %s", "сервер: %s", "服务器: %s", "サーバー: %s", "서버: %s" },
    { "friends on your LAN join at this address:", "друзья в вашей сети заходят по этому адресу:", "局域网朋友用此地址加入:", "LANの友達はこのアドレスで参加:", "같은 LAN 친구는 이 주소로 접속:" },
    { "friends over the internet: forward port %d on your router to this PC", "друзьям из интернета: пробросьте порт %d на роутере на этот ПК", "外网朋友: 在路由器上转发端口 %d 到本机", "インターネットの友達: ルーターでポート %d をこのPCへ転送", "인터넷 친구: 라우터에서 포트 %d를 이 PC로 포워딩" },
    { "players online: %d of %d", "игроков в сети: %d из %d", "在线玩家: %d / %d", "オンライン: %d / %d", "접속 플레이어: %d / %d" },
    { "server.ini next to the game edits port / max players / name", "server.ini рядом с игрой правит порт / максимум игроков / имя", "游戏旁的 server.ini 可改端口 / 人数 / 名称", "ゲーム横の server.ini でポート / 人数 / 名前を編集", "게임 옆 server.ini에서 포트 / 최대 플레이어 / 이름 편집" },
    { "F6 - close", "F6 - закрыть", "F6 - 关闭", "F6 - 閉じる", "F6 - 닫기" },
    { "HOST SETTINGS - saved to server.ini next to the game", "НАСТРОЙКИ ХОСТА - сохраняются в server.ini рядом с игрой", "主机设置 - 保存到游戏旁的 server.ini", "ホスト設定 - ゲーム横の server.ini に保存", "호스트 설정 - 게임 옆 server.ini에 저장" },
    { "server name", "имя сервера", "服务器名称", "サーバー名", "서버 이름" },
    { "port", "порт", "端口", "ポート", "포트" },
    { "max players", "максимум игроков", "最大玩家", "最大プレイヤー", "최대 플레이어" },
    { "friends type  <your ip>:<port>  on their Login screen", "друзья вводят  <ваш ip>:<порт>  на экране входа", "朋友在登录界面输入 <你的ip>:<端口>", "友達はログイン画面で <あなたのip>:<ポート> を入力", "친구는 로그인 화면에 <당신의 ip>:<포트> 입력" },
    { "Void Glowcaps", "Пустотные светляки", "虚空荧光菇", "ヴォイド光キノコ", "공허 광버섯" },
    { "Cinder Trumpet", "Тлеющий трубач", "余烬喇叭菇", "燃え殻トランペットキノコ", "재빛 나팔버섯" },
    { "Frost Puffball", "Морозный дождевик", "霜纹马勃", "霜のホコリタケ", "서리 먼지버섯" },
    { "Gaze Scroll", "Свиток взора", "凝视卷轴", "視線の巻物", "응시의 두루마리" },
    { "%s MUSHROOM x%d - pin it to a quick slot to eat   I - SATCHEL", "%s ГРИБ x%d - закрепи в быстрый слот, чтобы съесть   I - СУМКА", "%s 蘑菇x%d - 固定到快捷栏食用   I - 行囊", "%s キノコx%d - クイックスロットに登録して食べる   I - 袋", "%s 버섯 x%d - 빠른 슬롯에 지정해 먹기   I - 가방" },
    { "E - pick a mushroom in the field      1-6 - use a quick slot", "E - подобрать гриб в поле      1-6 - применить быстрый слот", "E - 采摘蘑菇      1-6 - 使用快捷栏", "E - キノコを採る      1-6 - クイックスロット使用", "E - 버섯 채집      1-6 - 빠른 슬롯 사용" },
    { "Click an item, then a slot below to pin it to 1-6.", "Кликни предмет, затем слот внизу - закрепить за 1-6.", "点击物品, 再点下方格子固定到1-6。", "アイテムをクリックし、下のスロット(1-6)に登録。", "아이템을 클릭한 뒤 아래 슬롯에 지정하세요 (1-6)." },
    ...
    /* ---- chat / events ---- */
    { "A rain cloud drifts in over the islands... mushrooms follow its rain.", "Дождевое облако приходит над островами... за его дождём растут грибы.", "一朵雨云飘到群岛上空……雨后蘑菇生长。", "雨雲が島々に流れ込む……その雨の後にキノコが生える。", "비구름이 섬들 위로 흘러든다…… 그 비를 따라 버섯이 자란다." },
    { "The rain cloud returns.", "Дождевое облако возвращается.", "雨云回来了。", "雨雲が戻ってきた。", "비구름이 돌아왔다." },
    ...
    { "The Warden of the Meadow opens its eyes.", "Страж Луга открывает глаза.", "草甸的守护者睁开了双眼。", "草地の守護者は目を開く。", "초원의 파수꾼이 눈을 뜬다." },
    { "The Warden's right arm tears away!", "Правая рука Стража оторвана!", "守护者的右臂被撕断了！", "守護者の右腕がもぎ取られた！", "파수꾼의 오른팔이 뜯겨 나갔다!" },
    { "The Warden's left arm tears away!", "Левая рука Стража оторвана!", "守护者的左臂被撕断了！", "守護者の左腕がもぎ取られた！", "파수꾼의 왼팔이 뜯겨 나갔다!" },
    { "The Warden's chest core is exposed!", "Ядро в груди Стража обнажено!", "守护者胸前的核心暴露了！", "守護者の胸のコアが露出した！", "파수꾼의 가슴 코어가 노출됐다!" },
    { "The Warden falls! Its wreckage condenses - warp cores hum in the grass.", "Страж пал! Обломки сгущаются - варп-ядра гудят в траве.", "守护者倒下了！残骸凝聚——曲率核心在草间嗡鸣。", "守護者は倒れた！残骸が凝り、歪曲コアが草の中で唸る。", "파수꾼이 쓰러졌다! 잔해가 응축되어 warp 코어가 풀 속에서 울린다." },
    { "WARDEN OF THE MEADOW", "СТРАЖ ЛУГА", "草甸守护者", "草地の守護者", "초원의 파수꾼" },
    ...
    /* ---- v59.8: radio stations ---- */
    { "Wanderer's March", "Марш странника", "流浪者进行曲", "放浪者の行進", "방랑자의 행진" },
    { "The Abyss", "Бездна", "深渊", "深淵", "심연" },
    { "Wraith Procession", "Шествие призраков", "亡灵行进", "亡霊の行列", "망자의 행렬" },
    { "Frozen Chapel", "Ледяная капелла", "冰封礼拜堂", "氷の礼拝堂", "얼어붙은 예배당" },
    { "Catacomb Vigil", "Катомбная вигилия", "地下墓穴守夜", "カタコンベの夜警", "카타콤 야경" },
    { "Ghost of the Green", "Призрак зелени", "绿荫之魂", "緑の亡霊", "초록의 망령" },
    /* ---- v59.8: HUD ---- */
    { "VOID HUNTERS FELLED: %d", "ПОВЕРЖЕНО ОХОТНИКОВ: %d", "击败虚空猎手: %d", "虚空ハンター撃破: %d", "처치한 공허 사냥꾼: %d" },
    { "VOID SHARDS: %d", "ПУСТОТНЫЕ ОСКОЛКИ: %d", "虚空碎片: %d", "虚空シャード: %d", "공허 파편: %d" },
    { "VOID TIDE  %.0f", "ПУСТОТНЫЙ ПРИЛИВ  %.0f", "虚空之潮  %.0f", "虚空の潮  %.0f", "공허의 조류  %.0f" },
    { "THE VOID STIRS - TIDE IN %.0f", "ПУСТОТА НЕПОКОЙНА - ПРИЛИВ ЧЕРЕЗ %.0f", "虚空躁动 - 潮汐还有%.0f", "虚空ざわめく - 潮まで%.0f", "공허가 요동친다 - 조류까지 %.0f" },
    { "FLY MODE  Tab to walk  Space/Shift up/down", "ПОЛЁТ  Tab - ходьба  Space/Shift - вверх/вниз", "飞行模式  Tab行走  Space/Shift升降", "飛行モード  Tabで歩く  Space/Shiftで昇降", "비행 모드  Tab 걷기  Space/Shift 상승/하강" },
    { "%s WEB: hold SHIFT to reel   SPACE release   F detach", "%s ПАУТИНА: SHIFT - подтянуться   SPACE - отпустить   F - отцепить", "%s 蛛网: 按住SHIFT收回   SPACE松开   F脱离", "%s ウェブ: SHIFTで巻き取り   SPACE解除   F切断", "%s 거미줄: SHIFT 당기기   SPACE 놓기   F 분리" },
    { "%s E - WARP   B - UPGRADE LASER (5 shards)", "%s E - варп   B - кузня лазера (5 осколков)", "%s E -传送   B -锻造激光(5碎片)", "%s E -ワープ   B -レーザー鍛造(5シャード)", "%s E -워프   B -레이저 단조 (파편 5)" },
    { "%s G - EAT MUSHROOM x%d   I - SATCHEL", "%s G - съесть гриб x%d   I - сумка", "%s G -吃蘑菇x%d   I -背包", "%s G -キノコを食べるx%d   I -袋", "%s G -버섯 먹기 x%d   I -가방" },
    { "%s SPACE - LAUNCH from the pad", "%s SPACE - старт с площадки", "%s SPACE -从平台起飞", "%s SPACE -パッドから発進", "%s SPACE -발판에서 이륙" },
    { "%s F web   SPACE x2 jump   glide   SHIFT dash x%d   I - SATCHEL", "%s F - паутина   SPACE x2 - прыжок   парение   SHIFT - рывок x%d   I - сумка", "%s F蛛网   SPACE x2跳   滑翔   SHIFT冲刺x%d   I -背包", "%s Fウェブ   SPACE x2ジャンプ   滑空   SHIFTダッシュx%d   I -袋", "%s F 거미줄   SPACE x2 점프   활공   SHIFT 대시 x%d   I -가방" },
    { "%s dash recharges %.1f   I - SATCHEL", "%s рывок восстановится через %.1f   I - сумка", "%s 冲刺恢复%.1f   I -背包", "%s ダッシュ回復%.1f   I -袋", "%s 대시 충전 %.1f   I -가방" },
    { "Hunters felled: %d", "Повержено охотников: %d", "击败猎手: %d", "ハンター撃破数: %d", "처치한 사냥꾼: %d" },
    { "Nearby: %d crawlers, %d wisps, %d spiders", "Рядом: ползунов %d, огоньков %d, пауков %d", "附近: 爬行者%d, 幽灵%d, 蜘蛛%d", "付近: クローラー%d, ウィスプ%d, クモ%d", "주변: 크롤러 %d, 위스프 %d, 거미 %d" },
    /* ---- v59.8: events ---- */
    { "Something skitters across the island...", "Что-то мелькает по острову...", "有什么东西在岛上窜过……", "何かが島を走り抜ける……", "무언가가 섬을 가로질러 달린다……" },
    { "The hatchling collapses into shards.", "Детёныш рассыпается осколками.", "幼体碎裂成碎片。", "孵化体は破片と崩れる。", "새끼가 파편으로 무너진다." },
    { "The rain cloud rains itself out and drifts away.", "Дождевое облако вылилось и уплывает прочь.", "雨云下完了雨，飘走了。", "雨雲は雨を降らせ切り、流れ去る。", "비구름이 비를 다 쏟고 흘러간다." },
    { "The wisp releases its shards.", "Огонёк выпускает свои осколки.", "幽灵释放出碎片。", "ウィスプがシャードを放つ。", "위스프가 파편을 내놓는다." },
    { "The void stirs... a tide of hunters rises.", "Пустота непокойна... поднимается прилив охотников.", "虚空躁动……猎手之潮涌起。", "虚空がざわめく……ハンターの潮が沸く。", "공허가 요동친다…… 사냥꾼의 파도가 일어난다." },
    { "THE VOID TIDE RISES - survive!", "ПУСТОТНЫЙ ПРИЛИВ ПОДНЯЛСЯ - выживи!", "虚空之潮涨起 - 生存下去!", "虚空の潮が満ちる - 生き延びろ!", "공허의 조류가 밀려온다 - 살아남아라!" },
    { "The tide recedes. The void grants +2 vitality.", "Прилив отступает. Пустота дарует +2 к живучести.", "潮汐退去。虚空赐予+2生命。", "潮が引く。虚空が+2の活力を与える。", "조류가 물러난다. 공허가 활력 +2를 준다." },
    { "The cocoon cracks open - empty. Shards drift out.", "Кокон трескается - пустой. Выплывают осколки.", "茧裂开 - 是空的。飘出碎片。", "繭が割れる - 中は空。シャードが漂う。", "고치가 갈라진다 - 비어 있다. 파편이 흘러나온다." },
};
